...
void main() {
  group('OsShareReceiverService', () {
    ...
    test('rejects private or localhost URLs', () async {
      ...
      final payload = await service.parseShareData(
        text:
            'Private link http://localhost:8080/test and http://192.168.1.1/admin',
      );
    ...
    test('validates file size limit', () {
      ...
      expect(
        service.validateFileSize(fileSizeBytes: 100 * 1024 * 1024),
        isTrue,
      );
      expect(
        service.validateFileSize(fileSizeBytes: 101 * 1024 * 1024),
        isFalse,
      );
    });

    test(
      'parses file attachments below size limit and skips oversized or non-existent files',
      () async {
    ...
    test(
      'emitShareData parses, emits on stream, and returns payload',
      () async {
        final service = OsShareReceiverService();
