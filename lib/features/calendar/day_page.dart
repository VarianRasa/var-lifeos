...
class _DayPageState extends ConsumerState<DayPage> with WidgetsBindingObserver {
  ...
  Future<void> _autoTimeBlockNodes(List<MindmapNode> nodes) async {
    final assignments = autoTimeBlockDay(dayNodes: nodes);
    if (assignments.isEmpty) {
      _showSnackBar('No unallocated tasks to time-block.');
      return;
    }

    final mutation = ref.read(mindmapMutationControllerProvider);
    for (final entry in assignments.entries) {
      final node = nodes.firstWhere((candidate) => candidate.id == entry.key);
      await mutation.scheduleNode(node, entry.value);
    }
    if (mounted) {
      _showSnackBar('Auto time-blocked ${assignments.length} tasks!');
    }
  }
            ...
            body: nodes.when(
              data: (value) {
                ...
                final filteredCanvasNodes = canvasNodes
                    ...
                    .toList();
                final visibleCanvasNodes = filteredCanvasNodes;
                                          ...
                                          children: [
                                            Positioned.fill(
                                              child: MindmapCanvas(
                                                ...
                                                onAutoTimeBlockRequested: () =>
                                                    _autoTimeBlockNodes(value),
